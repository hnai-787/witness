...
## 2026-09-09: Added e2e/ — a real Selenium suite retargeted at Project #7
...
retargets entirely at Project #7 (the Secure Student Management System,
...
1. **A real bug in Project #7 itself**: the per-account login rate
   ...
   whole route. Fixed in Project #7's own `app.py`
   (`methods=["POST"]` added to that limiter), with a new regression
   test there. See Project #7's own `PROJECT_NOTES.md` for its side of
   this story.
...
3. **A native browser `confirm()` dialog left unhandled.** Deleting a
   student triggers `onsubmit="return confirm(...)"` in Project #7's own
...
4. **A wait condition that was a no-op.** `DashboardPage.search()`
   ...
   instead of zero, which looked at first like a real Project #7
...
login once, then browse/search/paginate) found that Project #7's
...
capacity characteristic of Project #7's current configuration, reported
...
Project #7's own deliberate but conservative app-wide default).
...
This suite tests a separate, independent repository (Project #7), and
...
substituting for Project #7's own POST-as-Staff authorization test.

...
## 2026-09-09: Added e2e/ — a real Selenium suite retargeted at Rollkeeper
...
1. **A real bug in Rollkeeper itself**: the per-account login rate
   ...
   whole route. Fixed in Rollkeeper's own `app.py`
   (`methods=["POST"]` added to that limiter), with a new regression
   test there. See Rollkeeper's own `PROJECT_NOTES.md` for its side of
   this story.
...
3. **A native browser `confirm()` dialog left unhandled.** Deleting a
   student triggers `onsubmit="return confirm(...)"` in Rollkeeper's own
...
4. **A wait condition that was a no-op.** `DashboardPage.search()`
   ...
   instead of zero, which looked at first like a real Rollkeeper
...
login once, then browse/search/paginate) found that Rollkeeper's
...
capacity characteristic of Rollkeeper's current configuration, reported
...
Rollkeeper's own deliberate but conservative app-wide default).
...
This suite tests a separate, independent repository (Rollkeeper), and
...
substituting for Rollkeeper's own POST-as-Staff authorization test.

# Witness — real browser verification for Rollkeeper

A version-controlled Selenium WebDriver + pytest suite that exercises
the actual rendered browser experience of Rollkeeper (the Secure
Student Management System): login, CRUD, search, pagination, and —
the part that's genuinely complementary to Rollkeeper's own pytest
...
```text
Rollkeeper Flask application (a separate, independent repo)
        |
        +-- pytest / Flask test client -------- backend/security verification (Rollkeeper's own suite)
        |
        +-- live HTTP server
                |
                v
...
# start Rollkeeper (see scripts/run_e2e.sh for the full seed+start+run flow)
...
- **Absence, not disabled, for forbidden UI** — Rollkeeper's intended
...
meaning Rollkeeper's per-account limiter (5 attempts per 5 minutes,
...
**Fixed in Rollkeeper itself** (`app.py`, `@limiter.limit(..., key_func=
_account_rate_limit_key, methods=["POST"])`), with a new regression test
in Rollkeeper's own suite (`test_repeated_get_requests_to_login_do_not_
...
All 25 tests pass against a real, live, freshly-seeded Rollkeeper
...
of what they found — including a genuine capacity finding (Rollkeeper's
...
- Tests against Rollkeeper do not share a per-test transactional
  rollback (Rollkeeper has no such mechanism) — the E2E database is
...
- The forced-browsing GET-only `/delete/<id>` check documents a real
  ...
  denied) rather than substituting for Rollkeeper's own POST-as-Staff

# Project #13 E2E Suite — real browser verification for Project #7

A version-controlled Selenium WebDriver + pytest suite that exercises
the actual rendered browser experience of Project #7 (the Secure
Student Management System): login, CRUD, search, pagination, and —
the part that's genuinely complementary to Project #7's own pytest
...
```text
Project #7 Flask application (a separate, independent repo)
        |
        +-- pytest / Flask test client -------- backend/security verification (Project #7's own suite)
        |
        +-- live HTTP server
                |
                v
...
rebuilt suite targets a locally controlled application (Project #7,
...
# start Project #7 (see scripts/run_e2e.sh for the full seed+start+run flow)
...
- **Absence, not disabled, for forbidden UI** — Project #7's intended
...
meaning Project #7's per-account limiter (5 attempts per 5 minutes,
...
**Fixed in Project #7 itself** (`app.py`, `@limiter.limit(..., key_func=
_account_rate_limit_key, methods=["POST"])`), with a new regression test
in Project #7's own suite (`test_repeated_get_requests_to_login_do_not_
...
All 25 tests pass against a real, live, freshly-seeded Project #7
...
of what they found — including a genuine capacity finding (Project #7's
...
- Tests against Project #7 do not share a per-test transactional
  rollback (Project #7 has no such mechanism) — the E2E database is
...
- The forced-browsing GET-only `/delete/<id>` check documents a real
  ...
  denied) rather than substituting for Project #7's own POST-as-Staff
